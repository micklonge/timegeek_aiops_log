...
import log.normalization.common.DatetimeUtil;
import log.normalization.common.TimezoneUtil;
import log.normalization.common.LangFormatLogImpl;
...
public class LangFormatAggregatorFSM extends LangFormatLogImpl {
			...
			for (LangFormatAggregatorDataItem dataItem : dataGroup.dataItemList) {
				...
				case FunctionName:
					...
					if (dataItem.value.equals("year") == true) {
						logData.append(DatetimeUtil.getCurrentYear(TimezoneUtil.getChinaTimeZone()));
					} else if (dataItem.value.equals("month") == true) {
						logData.append(DatetimeUtil.getCurrentMonth(TimezoneUtil.getChinaTimeZone()));
					} else if (dataItem.value.equals("day")) {
						logData.append(DatetimeUtil.getCurrentDay(TimezoneUtil.getChinaTimeZone()));
					} else if (dataItem.value.equals("datetime")) {
						logData.append(DatetimeUtil.getCurrentDateTime(TimezoneUtil.getChinaTimeZone()));
					}
					break;
				default:
	...
	private static class LangFormatAggregatorDataItem extends LangFormatLogImpl {
		...
		public void logPrint(int length) {
			...
			case None:
				System.out.println("    None is Exception");
				break;
			case Value:
			...
			case Variable:
				...
				break;
			case FunctionName:
				System.out.println("    FunctionName = " + value);
				break;
			default:
